...
function GalleryCell({ item, sizes }: { item: GalleryItem; sizes: string }) {
  return (
    <label className="relative overflow-hidden group cursor-pointer w-full h-full block">
      ...
      <Image
        ...
        fill
        sizes={sizes}
        className="object-cover transition-all duration-700 group-hover:scale-105 group-hover:blur-[1px] max-md:peer-checked:scale-105 max-md:peer-checked:blur-[1px]"
      />
      {/* Dark gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 max-md:peer-checked:opacity-100 transition-opacity duration-500" />
      {/* Title slides up on hover or toggle (mobile) */}
      <div className="absolute bottom-0 left-0 w-full !p-6 md:!p-8 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 max-md:peer-checked:translate-y-0 max-md:peer-checked:opacity-100 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
